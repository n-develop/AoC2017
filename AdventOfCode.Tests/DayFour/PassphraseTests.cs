...
namespace AdventOfCode.Tests.DayFour
{
    public class PassphraseTests
    {
        ...
        public PassphraseTests(ITestOutputHelper output)
        {
            _output = output;
        }

        [Theory]
        [InlineData("aa bb cc dd aaa")]
        [InlineData("aa bb cc dd ee")]
        [InlineData("aa bb cc dd ee ff")]
        public void ValidPassphare(string input)
        {
            var passphrase = new Passphrase(input);

            Assert.True(passphrase.IsValid);
        }

        [Fact]
        public void NotValid()
        {
            var passphrase = new Passphrase("aa bb cc dd aa");

            Assert.False(passphrase.IsValid);
        }

        [Fact]
        public void ValidRealInput()
        {
            string inputs = ReadInput();
            var lines = inputs.Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries);
            var counter = 0;

            foreach (var line in lines)
            {
                var passphrase = new Passphrase(line);
            ...
            _output.WriteLine($"{counter} valid passphrases");
        }

        [Theory]
        [InlineData("abcde fghij")]
        [InlineData("a ab abc abd abf abj")]
        [InlineData("iiii oiii ooii oooi oooo")]
        public void SecurePassphare(string input)
        {
            var passphrase = new Passphrase(input);

            Assert.True(passphrase.IsSecure);
        }

        [Theory]
        [InlineData("abcde xyz ecdab")]
        [InlineData("oiii ioii iioi iiio")]
        public void NotSecurePassphrase(string input)
        {
            var passphrase = new Passphrase(input);

            Assert.False(passphrase.IsSecure);
        }

        [Fact]
        public void SecureRealInput()
        {
            string inputs = ReadInput();
            var lines = inputs.Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries);
            var counter = 0;
